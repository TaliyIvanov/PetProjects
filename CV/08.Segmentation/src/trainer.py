...
def train(cfg: DictConfig) -> None:
    ...
    train_imgs, temp_imgs, train_masks, temp_masks = train_test_split(
        image_paths, masks_paths, test_size=cfg.data.val_size, random_state=cfg.data.random_state
    )
    val_imgs, test_imgs, val_masks, test_masks = train_test_split(
        temp_imgs, temp_masks, test_size=cfg.data.test_size, random_state=cfg.data.random_state
    )

    # datasets
    train_dataset = SegmentationDataset(
        train_imgs, train_masks, transform=hydra.utils.instantiate(cfg.transforms.train_transforms)
    )
    val_dataset = SegmentationDataset(
        val_imgs, val_masks, transform=hydra.utils.instantiate(cfg.transforms.val_test_transforms)
    )
    ...
    def validate_epoch(model, dataloader, loss_fn, device, logger, step) -> dict:
        ...
        with torch.no_grad():
            for (
                images,
                masks,
            ) in tqdm(dataloader, desc=f"[Epoch {step + 1}/{cfg.train.epochs}] Val"):
                images = images.to(device)
                masks = masks.to(device)
                masks = masks.unsqueeze(1).float()
    ...
    for epoch in range(cfg.train.epochs):
        train_epoch(model, train_loader, optimizer, loss_fn, device, logger, epoch)
        ...
        if metrics["val_iou"] > best_iou:
            ...
            print(
                f"model saved to {os.getcwd()}/{model_filename} (IoU improved to {best_iou:.4f})"
            )
